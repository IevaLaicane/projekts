...
# Takes NASA API key and url from config file	
    nasa_api_key = config.get('nasa', 'api_key')
    nasa_api_url = config.get('nasa', 'api_url')
# Takes mysql configuration from config file
...
# Function for connection start with database
...
# Function to put asteroids information to database
...
if __name__ == "__main__":
	...
	if r.status_code == 200:
		# Parsing data from json fromat in arrays
		json_data = json.loads(r.text)
		#Creation of arrays according to types of asteroids
		ast_safe = []
		ast_hazardous = []
			...
			if ast_count > 0:
				for val in json_data['near_earth_objects'][request_date]:
					...
					if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
						...
						tmp_ast_id = val['id']
						#Geting estimated diameter of asteroids
						if 'kilometers' in val['estimated_diameter']:
							if 'estimated_diameter_min' and 'estimated_diameter_max' in val['estimated_diameter']['kilometers']:
								tmp_ast_diam_min = round(val['estimated_diameter']['kilometers']['estimated_diameter_min'], 3)
								tmp_ast_diam_max = round(val['estimated_diameter']['kilometers']['estimated_diameter_max'], 3)
							# Gives default diameter values if values are not available
							else:
								tmp_ast_diam_min = -2
								tmp_ast_diam_max = -2
						# Gives default diameter values if values are not available	
						...
						if len(val['close_approach_data']) > 0:
							if 'epoch_date_close_approach' and 'relative_velocity' and 'miss_distance' in val['close_approach_data'][0]:
								...
								# Checks if km per h information is available
								if 'kilometers_per_hour' in val['close_approach_data'][0]['relative_velocity']:
									tmp_ast_speed = int(float(val['close_approach_data'][0]['relative_velocity']['kilometers_per_hour']))
								else:
									tmp_ast_speed = -1
								# Checks if km are available
								if 'kilometers' in val['close_approach_data'][0]['miss_distance']:
									tmp_ast_miss_dist = round(float(val['close_approach_data'][0]['miss_distance']['kilometers']), 3)
								else:
									tmp_ast_miss_dist = -1
							else:	
								#Gives default values if information is not available
								tmp_ast_close_appr_ts = -1
								tmp_ast_close_appr_dt_utc = "1969-12-31 23:59:59"
								tmp_ast_close_appr_dt = "1969-12-31 23:59:59"
						else:
							# Journaling information if not a close approach
						...
						# Journaling information about asteroids
						logger.info("------------------------------------------------------- >>")
				...
				# Journaling information if no asteroids on that date
				logger.info("No asteroids are going to hit earth today")
		# Journaling information about asteroid count on that day
		logger.info("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe)))
		# If statement for hazardous asteroids
		if len(ast_hazardous) > 0:
			# Sorting asteroinds by time
			ast_hazardous.sort(key = lambda x: x[4], reverse=False)
			# Journaling information about count of hazardous  asteroids
			...
			# Sorting asteroids by distance 
			ast_hazardous.sort(key = lambda x: x[8], reverse=False)
			# Journaling information about closes asteroid
			...
			# Journaling information if there are no close asteroids
			logger.info("No asteroids close passing earth today")
		if len(ast_safe) > 0:	
			# Writes data in database for safe asteroids
			push_asteroids_arrays_to_db(request_date, ast_safe, 0)
#Error message if no data is gotten from API
	else:	
		# 
